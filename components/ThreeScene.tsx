...
let RANDOM_IMAGE_SRC = "https://source.unsplash.com/random/?Panorama&1";
const VIDEO_SRC =
  "./_next/static/chunks/pages/360_vr_master_series___free_download___view_on_low_waterfall_with_nice_city (1080p).mp4";
...
let isCaching = false;
let isPlaneEnabled = false;
let isCameraSource = false;
let isVideoSource = false;
let isImageSource = true;
let isRandomImageSource = false;
let randomImageNum = 0;
...
const ThreeScene: React.FC<ThreeSceneProps> = ({ width, height }) => {
  ...
  const setupSource = (updateTexture: () => Promise<void>, type: string) => {
    ...
    if (type === "camera") {
      ...
      inferImage(updateTexture, IMAGE_SRC);
    } else if (type === "random panorama (upsplash)") {
      randomImageNum = Math.ceil(Math.random() * 1000);
      inferImage(updateTexture, RANDOM_IMAGE_SRC + randomImageNum);
    }
  };
  ...
  function inferImage(updateTexture: () => Promise<void>, SRC: string) {
  ...
  function sceneSetup() {
    ...
    planeMesh = new Mesh(new PlaneGeometry(10, 10), sphereMaterial);
    planeMesh.rotateX(-1.57);
    planeMesh.translateZ(-1);
    planeMesh.visible = isPlaneEnabled;
    scene.add(planeMesh);
    ...
    const skydomeGeometry = new SphereGeometry(
      SKYDOME_RADIUS,
      SKYDOME_WIDTH_SEGMENTS,
      SKYDOME_HEIGHT_SEGMENTS
    ).scale(SKYDOME_SCALE_FACTOR, 1, 1);
  ...
  function setupGui(updateTexture: () => Promise<void>) {
    ...
    const guiSettings = [
      // Each setting is an object that contains the name, initial value, range (if applicable), and a callback function for when the value is changed
      {
        ...
        value: brightness,
        range: [0, 200],
        callback: (value: number) => (brightness = value),
      },
      {
        name: "Contrast",
        value: contrast,
        range: [0, 200],
        callback: (value: number) => (contrast = value),
      },
      {
        name: "Saturation",
        value: saturation,
        range: [0, 2],
        callback: (value: number) => (saturation = value),
      },
      {
        name: "Tint",
        value: tintColor,
        type: "color",
        callback: (value: string) => (tintColor = value),
      },
      {
        name: "Caching",
        value: isCaching,
        type: "checkbox",
        callback: (value: boolean) => (isCaching = value),
      },
      {
        name: "Plane",
        value: isPlaneEnabled,
        type: "checkbox",
        callback: (value: boolean) => (planeMesh.visible = value),
      },
      {
        name: "Frame Skip",
        value: frameSkip,
        range: [0, 60],
        callback: (value: number) => (frameSkip = value),
      },
      {
    ...
    folder
      .add(sourceType.current, "type", [
        "camera",
        "video",
        "image",
        "random panorama (upsplash)",
      ])
      .name("Source")
      .onChange((value: string) => {
        ...
        isRandomImageSource = value === "random panorama (upsplash)";
  ...
  useEffect(() => {
    ...
    const updateTexture = async () => {
      if (ctx) {
        ...
        if (isCaching && SHCache.has(currentFrame)) {
          ...
          sphereMaterial.userData.SphericalHarmonicCoefficients.value =
            SHCache.get(currentFrame);
        }
        // If the source is a video
          ...
          if (SHCBufferRef.current.length > frameSmoothing)
            SHCBufferRef.current.shift();
        ...
        // For other sources
        else if (isCameraSource || isImageSource || isRandomImageSource) {
          // Set the Spherical Harmonic Coefficients to the inferred image data
          sphereMaterial.userData.SphericalHarmonicCoefficients.value =
            await inferImageData();
        }
      }
    };
    ...
    const animate = () => {
      ...
      if (!isImageSource && !isRandomImageSource) {
        drawImageWithFilters(
          ctx,
          source,
          brightness,
      ...
      if (!isImageSource && !isRandomImageSource && ++frameCount >= frameSkip) {
        updateTexture();
        frameCount = 0;
      }
    };
